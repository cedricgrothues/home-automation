...
class SceneService {
  ...
  static Future<List<Scene>> fetch() async {
    ...
    try {
      final response =
          await get('http://hub.local:4000/modules.scene/scenes').timeout(
    ...
    return scenes != null
        ? scenes.map((scene) => Scene.fromJson(scene)).toList()
        : <Scene>[];
  }
  ...
  static void run(Scene scene) async {
    try {
      final result = await get(
              'http://hub.local:4000/modules.scene/scenes/${scene.id}/run')
          .timeout(
        const Duration(seconds: 1),
      );
