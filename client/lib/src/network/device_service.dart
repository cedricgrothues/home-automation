...
class DeviceService {
  ...
  static Future<List<Device>> fetch() async {
    ...
    try {
      final response =
          await get('http://hub.local:4000/core.device-registry/devices')
              .timeout(
    ...
    for (final device in devices) {
      ...
      try {
        ...
        device['state'] =
            json.decode(response.body)['state'] ?? {'error': true};
      } on SocketException {
        // SocketExceptions are thrown if there appears to be a problem with the users internet connection
        // or if a DNS lookup failed (latter should not be a problem at this point sice we're working with ip addresses instead of urls)
    ...
    return devices != null
        ? devices.map((device) => Device.fromJson(device)).toList()
        : <Device>[];
  }

  /// `update` alters the device state based on the passed instance
  /// of [Device]. Should be called when pressing a device card.
  /// Returns the updated [DeviceState] instance.
  ...
  /// an updated instance. Should be used to rebuild the screen if the
  /// [DeviceState] changes.
